...
class WebDLMController {
    ...
    public function dlm_request($request) {
        ...
        if ($request->get_type() == "GET") {
            ...
            foreach ($connector_run as $dlm_id) {
                ...
                $result = $this->run_one_dlm($dlm_id, $request, $result);
    ...
    private function run_one_dlm($dlm_id, $request, $result) {
        $type = strtolower($request->get_type());
        $is_online = $this->dlms[$dlm_id]->is_ready();
        if ($is_online) {
            $result->set_dlm_result($dlm_id, $this->dlms[$dlm_id]->$type($request));
        } else {
