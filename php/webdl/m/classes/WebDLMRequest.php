...
class WebDLMRequest {
    ...
    public function set_type($type) {
        ...
        switch ($type) {
            ...
            case "DELETE":
                ...
                break;
            default:
                WebDLUserMessage::output('Request type '.$type.' not supported, please use GET, POST, or DELETE', 'WebDLMRequest.php');
                return false;
        }
        return true;
    }
    ...
    public function push_column($c_id, $c_val='') {
        ...
        if (isset($this->columns[$c_id]))
            WebDLUserMessage::output('Column '.$c_id.' has already been pushed to the DLM request.  Be careful if this is a POST request, as the last pushed value will be the only one saved.', 'WebDLMRequest.php');            
        $class = new stdClass();
        $class->c_id = $c_id;
        $class->c_val = $c_val;
    ...
    public function push_match($c_id, $m_val, $type='AND') {
        ...
        if ($type != 'AND' && $type != 'OR' && $type != 'WILDCARD') {
            WebDLUserMessage::output('Unsupported match type '.$type.', please use AND, OR, or WILDCARD', 'WebDLMRequest.php');
            return false;            
        }
        $class = new stdClass();
        $class->c_id = $c_id;
